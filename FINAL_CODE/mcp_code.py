...
def advance_time(hours: int = 0, minutes: int = 0) -> dict:
    ...
    classcode._time_offset += timedelta(hours=hours, minutes=minutes)
    return {
        "success":           True,
...
def reset_time() -> dict:
    ...
    classcode._time_offset = timedelta(0)
    return {
        "success":           True,
        "current_mock_time": now().strftime("%Y-%m-%d %H:%M:%S"),
    }
...
def get_current_time() -> dict:
    ...
    return {
        "success":           True,
        "current_mock_time": now().strftime("%Y-%m-%d %H:%M:%S"),
        "offset":            str(classcode._time_offset),
    }
...
def register_account(
    ...
    """
    ขั้นตอนแรกของการสมัครสมาชิก RhythmReserve
 
    membership เลือกได้ 3 แบบ:
    - STANDARD : ฟรี | ส่วนลด 0% | 3 pts/hr | ยกเลิกก่อน 24 ชม.
    - PREMIUM  : ฿299 | ส่วนลด 3% | 5 pts/hr | ยกเลิกก่อน 12 ชม.
    - DIAMOND  : ฿599 | ส่วนลด 5% | 8 pts/hr | ยกเลิกก่อน 6 ชม.
    birthday: YYYY-MM-DD
 
    ⚠️ IMPORTANT — Flow ที่ถูกต้องสำหรับ PREMIUM / DIAMOND:
    1. เรียก register_account → จะได้รับ require_payment=True (ยังไม่สมัครสำเร็จ)
    2. ถามผู้ใช้ว่าต้องการชำระด้วยวิธีใด ('credit' หรือ 'qr')
    3. เรียก pay_and_confirm_register พร้อมข้อมูลเดิมและ payment_method ที่เลือก
    4. เมื่อ pay_and_confirm_register สำเร็จ → จึงแจ้งผู้ใช้ว่า "สมัครสมาชิกสำเร็จ"
 
    ❌ ห้ามแจ้งผู้ใช้ว่าสมัครสำเร็จก่อนที่ pay_and_confirm_register จะคืน success=True
    """
    try:
        mu = membership.upper()
        ...
        # PREMIUM / DIAMOND → ต้องให้ผู้ใช้เลือกช่องทางชำระเงินก่อน แล้วค่อยเรียก pay_and_confirm_register
        # ยังไม่ได้สมัครสมาชิก — success=False เพื่อป้องกันการแจ้งผลก่อนกำหนด
        return {
            "success":         False,
            "require_payment": True,
            "registered":      False,
            "message":         f"membership {mu} มีค่าสมัคร ฿{mem_enum.price} — ยังไม่ได้สมัครสมาชิก กรุณาถามผู้ใช้ว่าต้องการชำระด้วยวิธีใดก่อน",
            ...
            "next_step": (
                "1. ถามผู้ใช้ว่าเลือกชำระด้วย 'credit' (บัตรเครดิต) หรือ 'qr' (QR Code)\n"
                "2. รอคำตอบจากผู้ใช้\n"
                "3. เรียก pay_and_confirm_register พร้อมข้อมูลเดิมทั้งหมดและ payment_method ที่ได้รับ\n"
                "4. แจ้งผู้ใช้ว่าสมัครสำเร็จก็ต่อเมื่อ pay_and_confirm_register คืน success=True เท่านั้น"
            ),
        }
...
def view_cancellation_policy(customer_id: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            "your_limit": f"คุณต้องยกเลิกก่อนอย่างน้อย {cancel_limit} ชม.",
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
#
# 6. COUPON - VIEW & REDEEM
#

@mcp.tool()
def view_my_coupons(customer_id: str) -> dict:
    """ดู Coupon ทั้งหมดในบัญชี (ใช้ได้ / หมดอายุ / ใช้แล้ว)"""
    try:
...
@mcp.tool()
def redeem_coupon(customer_id: str, points_to_spend: int) -> dict:
    """
    แลกแต้มเป็น Coupon ส่วนลด (manual redeem)
    Coupon มีอายุ 1 เดือนนับจากวันที่แลก

    ตัวเลือกตาม Membership:
    ┌──────────┬────────────────────────────────────────────┐
    │ Standard │ 20 pts → 5% off                           │
    │ Premium  │ 20 pts → 10% off  |  30 pts → 15% off    │
    │ Diamond  │ 20 pts → 10% off  |  30 pts → 15% off    │
    │          │ 40 pts → 20% off                          │
    └──────────┴────────────────────────────────────────────┘

    points_to_spend: จำนวนแต้มที่ต้องการใช้ (20 / 30 / 40)
    """
    try:
        customer = store.get_customer_by_id(customer_id)
        tier     = type(customer).__name__

        # ตรวจสอบตัวเลือกที่รองรับตาม tier
        options_map = {
            "Standard": {20: "5%"},
            "Premium":  {20: "10%", 30: "15%"},
            "Diamond":  {20: "10%", 30: "15%", 40: "20%"},
        }
        valid_options = options_map.get(tier, {})

        if points_to_spend not in valid_options:
            return {
                "success": False,
                "error":   f"ตัวเลือก {points_to_spend} แต้มไม่รองรับสำหรับ {tier}",
                "your_options": [
                    {"points": p, "discount": d} for p, d in valid_options.items()
                ],
                "your_current_points": customer.points,
            }

        if customer.points < points_to_spend:
            return {
                "success":             False,
                "error":               f"แต้มไม่เพียงพอ (มี {customer.points} แต้ม ต้องการ {points_to_spend} แต้ม)",
                "your_current_points": customer.points,
                "points_needed":       points_to_spend - customer.points,
            }

        coupon = store.manual_redeem_coupon(customer_id, points_to_spend)

        return {
            "success":              True,
            "message":              f"แลกคูปองสำเร็จ! ได้ส่วนลด {valid_options[points_to_spend]}",
            "coupon_id":            coupon.id,
            "discount":             f"{coupon.get_discount()*100:.0f}%",
            "expires":              str(coupon.get_expired_date().date()),
            "points_spent":         points_to_spend,
            "remaining_points":     customer.points,
            "how_to_use":           "ระบุ coupon_id ใน pay_reservation หรือ view_price_breakdown",
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def view_points(customer_id: str) -> dict:
    ...
    try:
        ...
        redeem_options = {
            "Standard": [{"pts": 20, "discount": "5%",  "note": "auto หรือ manual redeem"}],
            "Premium":  [{"pts": 20, "discount": "10%", "note": "auto หรือ manual redeem"},
                         {"pts": 30, "discount": "15%", "note": "manual redeem เท่านั้น"}],
            "Diamond":  [{"pts": 20, "discount": "10%", "note": "auto หรือ manual redeem"},
                         {"pts": 30, "discount": "15%", "note": "manual redeem เท่านั้น"},
                         {"pts": 40, "discount": "20%", "note": "manual redeem เท่านั้น"}],
        }
        ...
        return {
            ...
            "note":           "Auto-redeem จะเลือก tier สูงสุดที่ทำได้หลังชำระ Service IN | ใช้ redeem_coupon เพื่อ manual redeem",
        }
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def cancel_booking(customer_id: str, service_id: str, booking_id: str) -> dict:
    ...
    Cancel Policy ตาม Membership:
    - STANDARD : ต้องยกเลิกก่อน >= 24 ชม. → คืนเงิน 100%
    - PREMIUM  : ต้องยกเลิกก่อน >= 12 ชม. → คืนเงิน 100%
    - DIAMOND  : ต้องยกเลิกก่อน >=  6 ชม. → คืนเงิน 100%
    ยกเลิกช้ากว่า limit → ไม่ได้รับเงินคืน
    """
    try:
        customer      = store.get_customer_by_id(customer_id)
        service_in    = customer.get_reserve(service_id)
        ...
        return {
            ...
            "error":   str(e),
            "cancellation_policy": {
                "your_membership": tier,
                "your_limit_hrs":  cancel_limit,
                "STANDARD":        "ยกเลิกก่อน 24 ชม. → คืน 100%",
                "PREMIUM":         "ยกเลิกก่อน 12 ชม. → คืน 100%",
                "DIAMOND":         "ยกเลิกก่อน  6 ชม. → คืน 100%",
                "if_late":         "ไม่ได้รับเงินคืน",
            },
        }
...
#
# 9. INSPECT & REPORT DAMAGE (ก่อน checkout)
#
...
def inspect_before_checkout(
    ...
    try:
        ...
        if diff_min > 15:
            ...
            late_info = (
                f"⚠️ ออกช้า {diff_min:.0f} นาที "
                f"→ ค่าปรับ {rounded} ชม. × {booking.room.rate} ฿ = {rounded * booking.room.rate:.0f} ฿"
            )
        elif diff_min > 0:
        ...
        return {
            ...
            "equipment": [
                {
                    "eq_id":       eq.id,
                    "type":        eq.type,
                    "repair_cost": f"{eq.price} ฿ (ค่าปรับถ้าเสียหาย)",
                }
            ...
            "policy_reminder": {
                "late":      "grace 15 นาที → ค่าปรับ = room_rate × ชม.ที่เกิน (ปัดขึ้น)",
                "room":      "ค่าซ่อมห้อง = ค่าที่ Staff ประเมิน",
                "equipment": "ค่าซ่อมอุปกรณ์ = eq.price ต่อชิ้น (ดูจาก repair_cost ด้านบน)",
            },
            "next_step": (
    ...
    try:
        ...
        for eq in booking.eq_list:
            if eq.id in damaged_ids:
                ...
                eq_damage_summary.append({
                    "eq_id":       eq.id,
                    "type":        eq.type,
                    "repair_cost": f"{eq.price} ฿",
                    "status":      "⚠️ Damaged → MAINTENANCE",
                })
            else:
                eq_damage_summary.append({
                    "eq_id":  eq.id,
        ...
        return {
            ...
            "damages": {
                ...
                "room_cost":      f"{room_damage_cost} ฿" if is_room_damaged else "-",
...
def checkout(customer_id: str, service_id: str, booking_id: str,payment_method: str,) -> dict:
    ...
    3. inspect_before_checkout
    4. report_damage (ถ้ามีของเสียหาย)
    5. checkout ← ชำระเงิน

    payment_method: 'credit' = บัตรเครดิต, 'qr' = QR Code

    Late Checkout Policy (คิดอัตโนมัติ):
    - grace period 15 นาที → ไม่คิดค่าปรับ
    - เกิน 15 นาที → ค่าปรับ = room_rate × ชม.ที่เกิน (ปัดขึ้น)
    """
    try:
        ...
        room_already_set = any(
            p.type == PenaltyType.DAMAGE and "Room" in p.reason
            for p in service_out.penalty_list
        )
        if not room_already_set:
            booking.room.update_timeslot_status(
                booking.day, booking.start, booking.end,
                RoomEquipmentStatus.AVAILABLE)

        damaged_eq_reasons = {
            p.reason for p in service_out.penalty_list
            if p.type == PenaltyType.DAMAGE
        }
        for eq in booking.eq_list:
            eq_already_damaged = any(eq.id in reason for reason in damaged_eq_reasons)
            if not eq_already_damaged:
                eq.update_timeslot_status(
                    booking.day, booking.start, booking.end,
                    RoomEquipmentStatus.AVAILABLE)
